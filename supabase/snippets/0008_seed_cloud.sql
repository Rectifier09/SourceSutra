-- ============================================================================
-- CLOUD SEED — migration 0008 companion, for the DEPLOYED Supabase project.
-- Paste this whole file into the Supabase dashboard SQL Editor and Run.
--   (`supabase db push` applies the migration but NOT seeds on a remote DB.)
--
-- The 12 prototype suppliers from SourceSutraCustomer.dc.html, enriched + verified
-- so the buyer Discover screen shows the real directory. Anand Knitfab & Ludhiana
-- Woolworks already exist in cloud (loginable, verified); the other 10 are added as
-- directory-only demo suppliers — no auth user needed, since "verified" derives from
-- onboarding_sections, not a login. Tiruppur Threads is untouched.
--
-- SAFE TO RE-RUN: org inserts are guarded (skip existing names), onboarding upserts,
-- and certs are deleted-then-reinserted for the 6 rich suppliers. Runs as the SQL
-- Editor's admin role, so the reviewer escape hatch + section_guard behave as in reset.
-- ============================================================================

-- ── 10 new supplier orgs (Anand Knitfab / Ludhiana Woolworks already in cloud) ──
-- Guarded insert: only names that don't already exist (orgs.name has no unique key).
insert into orgs (kind, name, location)
select 'supplier', v.name, v.location
from (values
  ('Bhilwara Processors','Bhilwara, Rajasthan'),
  ('Erode Textile Exports','Erode, Tamil Nadu'),
  ('Surat Silk Mills','Surat, Gujarat'),
  ('Panipat Home Textiles','Panipat, Haryana'),
  ('Tirupur Trims & Accessories','Tiruppur, Tamil Nadu'),
  ('Ludhiana Embroidery House','Ludhiana, Punjab'),
  ('Surat Screen Print Co.','Surat, Gujarat'),
  ('Erode Spinning Mills','Erode, Tamil Nadu'),
  ('Bhilwara Suiting Weavers','Bhilwara, Rajasthan'),
  ('Panipat Trims Co.','Panipat, Haryana')
) as v(name, location)
where not exists (select 1 from orgs o where o.kind = 'supplier' and o.name = v.name);
...
begin
  -- refresh certs for all rich suppliers to the prototype set (idempotent re-run)
  delete from certifications where org_id in (s1, s3, s4, s6, s8, s10);
